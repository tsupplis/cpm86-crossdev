  ...
  struct node *args;
{
    ...
    oldstk = xlsave(&arg,&list,&val,0);

    /* initialize */
    arg.n_ptr = args;

    /* evaluate and append each argument */
    for (last = 0; arg.n_ptr != 0; last = lptr) {
  ...
  struct node *args;
{
    ...
    /* get the argument */
    if ((arg = xlevarg(&args)) == 0 || arg->n_type != LIST)
	return (t);
    else
	return (0);
}
...
{
    /* compare the arguments */
    if (arg1 != 0 && arg1->n_type == INT &&
    	arg2 != 0 && arg2->n_type == INT)
  ...
  struct node *args;
{
    ...
    /* get the list */
    if ((list.n_ptr = xlevmatch(LIST,&arg.n_ptr)) == 0)
	xlfail("invalid argument");
	...
	if (list.n_ptr == 0 || list.n_ptr->n_type != LIST)
	    xlfail("invalid argument");
    }
  ...
  struct node *args;
{
    ...
    /* create a new stack frame */
    oldstk = xlsave(&arg,&list,&last,&val,0);
	...
	while (list.n_ptr != 0 && list.n_ptr->n_type == LIST) {
