  ...
  int type; struct node **pargs;
{
    ...
    /* check its type */
    if (type == LIST) {
	if (arg != 0 && arg->n_type != LIST)
	    xlfail("bad argument type");
    }
    else {
	if (arg == 0 || arg->n_type != type)
	    xlfail("bad argument type");
    }
  ...
  struct node *args;
{
    ...
    /* create a new stack frame */
    oldstk = xlsave(&arg,&sym,&fargs,&fun,0);
  ...
  struct node *args;
{
    ...
    /* create a new stack frame */
    oldstk = xlsave(&arg,&sym,&list,&code,&oldbnd,0);
  ...
  struct node *args;
{
    ...
    /* create a new stack frame */
    oldstk = xlsave(&arg,&testexpr,&thenexpr,&elseexpr,0);
    ...
    if (dothen)
	while (thenexpr.n_ptr != 0)
	    val = xlevarg(&thenexpr.n_ptr);
    else
	while (elseexpr.n_ptr != 0)
