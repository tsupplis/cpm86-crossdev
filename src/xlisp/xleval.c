  ...
  struct node *nptr;
{
    ...
    if ((fun.n_ptr = xleval(fun.n_ptr)) == 0)
  ...
  struct node *fun,*args;
{
    ...
    /* create a stack frame */
    oldstk = xlsave(&cptr,0);

    /* get the formal argument list */
    if ((fargs = fun->n_listvalue) != 0 && fargs->n_type != LIST)
    ...
    /* execute the code */
    for (cptr.n_ptr = fun->n_listnext; cptr.n_ptr != 0; )
  ...
  struct node *fargs,*aargs;
{
    ...
    while (farg.n_ptr != 0 && aarg.n_ptr != 0) {
    ...
    if (farg.n_ptr != 0 && farg.n_ptr->n_listvalue == slash)
	while ((farg.n_ptr = farg.n_ptr->n_listnext) != 0)
	    xlbind(farg.n_ptr->n_listvalue,0);
