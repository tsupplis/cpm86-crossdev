...
static struct node *(*newkmap())[]
{
    ...
    /* allocate the vector */
    map = (struct node *(*)[]) calloc(1,sizeof(struct node *) * KMSIZE);
    if (!map) {
  ...
  struct node *args;
{
	...
	if (*sptr == 0)
	    ...
	    if ((*map)[ch] == 0 || (*map)[ch]->n_type != KMAP) {
		(*map)[ch] = newnode(KMAP);
  ...
  struct node *args;
{
	...
	if ((map = kmptr->n_kmap) == 0)
	    ...
	    keys[kndx] = 0;
	    if (sendmsg(nptr,currentenv->n_symvalue,margs.n_ptr) == 0)
		break;
	    kmptr = kmap;
	    kndx = 0;
	}
  ...
  struct node *msym,*env,*args;
{
    ...
    for (eptr = env; eptr != 0; eptr = eptr->n_listnext)
	if ((obj = eptr->n_listvalue) != 0 && obj->n_type == OBJ)
	    if ((msg = xlmfind(obj,msym)) != 0)
