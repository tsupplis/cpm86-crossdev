...
static struct node *plist()
{
    ...
    for (lastnptr = 0; (ch = nextch()) > 0 && ch != ')'; lastnptr = nptr) {
  ...
  char *sname;
{
    ...
    for (sptr = oblist->n_symvalue; sptr != 0; sptr = sptr->n_listnext)
	if (sptr->n_listvalue == 0)
	    printf("bad oblist\n");
	else if (sptr->n_listvalue->n_symname == 0)
