  ...
  struct node *obj,*msg,*args;
{
    ...
    oldstk = xlsave(&method,&cptr,&val,0);
    ...
    if (msg->n_msg == new && val.n_ptr != 0) {
	if ((isnewmsg = xlmfind(val.n_ptr,isnew)) == 0)
  ...
  struct node *obj,*args;
{
    ...
    if ((msg = xlmfind(obj,xlevmatch(SYM,&args))) == 0)
  ...
  struct node *sym;
{
    ...
    if ((obj = self->n_symvalue) != 0 && obj->n_type == OBJ)
  ...
  struct node *cls,*msg;
{
    ...
    /* lookup the message */
    for (lptr = ivar->n_listvalue; lptr != 0; lptr = lptr->n_listnext)
  ...
  struct node *args;
{
    ...
    /* create a new stack frame */
    oldstk = xlsave(&arg,&msg,&fargs,&code,0);
  ...
  struct node *args;
{
    ...
    if ((super = xlivar(cls,SUPERCLASS)->n_listvalue) != 0)
  ...
  struct node *cls; int ivar;
{
    ...
    if ((cnt = xlivar(cls,ivar)->n_listvalue) != 0)
	if (cnt->n_type == INT)
	    return (cnt->n_int);
	else
  ...
  struct node *args;
{
    ...
    /* find the message binding for this message */
    if ((msg = findmsg(super,xlevmatch(SYM,&args))) == 0)
	xlfail("no method for this message");
  ...
  struct node *cls,*sym;
{
	...
	for (lptr = xlivar(msgcls,MESSAGES)->n_listvalue;
	    ...
	    if ((msg = lptr->n_listvalue) != 0 && msg->n_msg == sym)
		return (msg);
  ...
  struct node *obj,*sym;
{
    ...
    /* find the variable */
    found = FALSE;
    for (; cls != 0; cls = xlivar(cls,SUPERCLASS)->n_listvalue) {
